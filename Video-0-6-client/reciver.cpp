...
void Reciver::getVideo(string filename,char* &out_filename){
...
//    int frame_index=0;
    char* in_filename;
    in_filename=const_cast<char *>(filename.c_str());
    av_register_all();
    //Network
...
//            printf( "Failed to retrieve input stream information");
    ...
    if (!ofmt_ctx) {
        cout<<"Could not create output context\n"<<endl;
//            printf( "Could not create output context\n");
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (!out_stream) {
            cout<<"Failed allocating output stream\n"<<endl;
...
//                printf( "Could not open output URL '%s'", out_filename[k]);
            goto end;
        }
    }
    //Write file header
...
//            cout<<"Receive"<<frame_index<<"video frames from input URL\n"<<endl;
